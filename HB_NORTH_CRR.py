...
def bug_filter():
    ...
    for datas in result_list:
        ...
        peakwd_for_date = sorted(
            peakwd_for_date, key=lambda x: int(x.get("sequence", 0)), reverse=True
        )
        peakwe_for_date = sorted(
            peakwe_for_date, key=lambda x: int(x.get("sequence", 0)), reverse=True
        )
        off_peak_for_date = sorted(
            off_peak_for_date, key=lambda x: int(x.get("sequence", 0)), reverse=True
        )
